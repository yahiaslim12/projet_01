...
export default function Cards() {
  ...
  const [detail,setDetail] = useState([
    ...
    {name : 'Succer',qte : 210 , change : false , modify : false},
    {name : 'Chocolat',qte : 11 , change : false , modify : true},
    {name : 'Smid',qte : 44 , change : false , modify : false},
    {name : 'Any thing',qte : 44 , change : false , modify : true},
         ...
         setDetail(prev =>(
            prev.map( x => {
                if(x.name === element.name){
                   if(e === 'open') return {...x , change : true}
                   else {
     ...
     <section className="cards">
                ...
                cards.map((card)=>(
                    <div key={card.id} className="border-top border-bottom d-flex justify-content-between align-items-center py-2">
                        ...
                        <div className="gap-2 d-none d-sm-flex">
                            <button onClick={()=>handleChange(card.id,'-')} className="btn border rounded">-</button>
                            <span className="border p-2 rounded d-flex align-items-center justify-content-center">
                                <p>{card.qte}</p>
                            </span>
                            <button onClick={()=>handleChange(card.id,'+')} className="btn border rounded">+</button>
                        </div>
                        <div className="d-flex gap-1 alignt-items-center">
        ...
        <Modal open = {open} onClose={()=>setOpen(false)}>
            <Box className = "cards_modal">
                ...
                <div className="d-flex justify-content-between border-bottom py-3 px-2">
  <div>
                    ...
                    <div className="table_detail border rounded">
                            ...
                            <div className="tbody" style={{height : '400px',overflowY : "auto"}}>
                                {
                                    ...
                                    <div key={index} className="tr border-bottom d-flex justify-content-between py-2">
                                            ...
                                            <span className="px-3 d-flex justify-content-center align-items-center" style={{color : colors.gray_p}}>
                                                {item && item.change ? <input value={item.qte} onChange={(e)=>handleModify(item,e)} style={{outline: 'none',width : "50px"}} type="text" className="border rounded px-2 py-1"/> : <p className="text-center" onDoubleClick={()=> handleModify(item,'open')}>360 g</p>}
                                            </span>
                                            <span className="px-3 d-flex justify-content-center align-items-center" style={{}}>
                                                <small className="rounded text-light fw-bold px-2 py-1" style={{backgroundColor : item && item.modify ? colors.primary : "red"}}>{item && item.modify ? 'You can modify' : 'You can not'}</small>
                                            </span>
                                    </div>
                            ...
                            <div className="tfoot d-flex justify-content-between border-bottom py-2 px-3">
                                ...
                                <div className="gap-2 d-flex">
                                    <button onClick={()=>handleChange(cardModal.id,'-')} className="btn border rounded">-</button>
                                    <span className="border p-2 rounded d-flex align-items-center justify-content-center">
